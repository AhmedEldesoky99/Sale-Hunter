                ...
                <div className = "row align-vertical">
                    ...
                    <div className="col-md-5 form-side">
                        <div className="settings-apperance">
                            <span  className={`settings-title ${ar?'ar': ''}`}> {ar? 'المظهر' : 'Apperance'}</span>
                            <div className="settings-apperance-options">
                                ...
                                <div  className={`settings-apperance-image ${!dark? "active": ''}`}
                                    ...
                                    <span className={ar ? 'ar' : ''}>{ar? 'الوضع المضئ' : 'Light mode'}</span>
                                </div>
                                ...
                                <div className={`settings-apperance-image ${dark? "active":''}`} 
                                    ...
                                    <span className={ar ? 'ar' : ''}> {ar? 'الوضع المظلم' : 'Dark mode'}</span>
                                </div>

                                <div className={`settings-apperance-image`} 
                                    onClick={() => systemDependTheme()}>
                                    <img src={systemTheme} alt="system-mode" />
                                    <span className={ar ? 'ar' : ''}> {ar? 'وضع النظام' : 'System mode'}</span>
                                </div>
                        ...
                        <div className="settings-language">
                            <span  className={`settings-title ${ar?'ar': ''}`}> {ar? 'اللغه' : 'Language'}</span>
                            <div className="settings-language-options">
                                ...
                                <div className="settings-language-option" onClick={()=> changeLanguage('ar')}>
                                    ...
                                    <label className="ar" htmlFor="arabic">اللغة االعربيه</label>
                                </div>  
                            </div>
                        </div>
                        <div className="settings-data">
                            <span  className={`settings-title ${ar?'ar': ''}`}> {ar? 'بيانات المستخدم' : 'User Data'}</span>
                            <div className="settings-data-options">
                                {props.isLoggedIn && 
                                    <button className={`settings-data-option bg-primary ${ar?'ar': ''}`} onClick={() =>logout()}>{ar?'تسجيل الخروج': 'Logout'}</button>
                                }
                                {!props.isLoggedIn && 
                                    <Link to ="/sign-in" className={`settings-data-option bg-primary ${ar?'ar': ''}`}> {ar? "تسجيل الدخول" : "Login"}</Link>
                                }
                                <button className={`settings-data-option bg-danger ${ar? "ar" :''}`} onClick={() => clearData()}>{ar? "مسح بياناتي" : 'Clear data'}</button>
                            </div>
                        </div>
